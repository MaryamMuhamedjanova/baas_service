...
def request_body():
    def _create_body(clientCode):
        current_body = data.getList_body.copy()
...
def service_response(token, request_body):
    def _get_response(clientCode):
        ...
        response = baas_request.service_getList_get(body, token)
...
@allure.suite("(GetList) Получение списка счетов клиента")
class TestSuite:
        ...
        with allure.step("Проверка тела ответа"):
            ...
            for account in response_data["data"]:
                validate_fields(account,
                                {"number", "bic", "bank", "currency", "clientCode", "status", "creationDate", "product",
                                 "balance"}, "записи")
